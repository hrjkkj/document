...
if (!GDT._inited) {
	...
	var PMT = {
		...
		var _httpsPreload = function() {
			...
			for (var i = 0,
			len = urls.length; i < len; i++) { (new Image()).src = urls[i];
			}
		};
		var cacheUrl = {};
		...
		fnQueue = {
			...
			add: function(key, cb) { ! fnQueue.fnlist[key] && (fnQueue.fnlist[key] = []);
			...
			a.lib = {
				...
				onComplete: function() {
					try {
						...
						a.exec();
					} catch(e) { (new Image()).src = "http://c.isdspeed.qq.com/code.cgi?domain=i.gdt.qq.com&cgi=/view.fcg?loaderr&type=1&time=1&rate=1&code=53&r=" + Math.random();
					}
				},
				load: function() {
					if (!a.lib._hasLoaded) {
